...
export default class Filter {
    MAX_ENTRIES_PER_BUCKET: number;
    MIN_ENTRIES_PER_BUCKET: number;
    filter: any;

    constructor(option: { storage_path?: any; isUpdate: any; }, ) {
    ...
    _init_update_filter(json_filter_path: any) {
        return this.create(30000, 0.001);
    }

    _init_first_filter(json_filter_path: any) {
        try {
    ...
    */
    ...
    optimalEntriesPerBucket(e: number) {
        // 1>e >0.0;
        if (e <= 0.00001) {
    ...
    */
    ...
    optimalBitsPerEntry(e: number, b: number) {
    ...
    */
    optimalNumberOfBuckets(n: number, b: number) {
        return Math.round(
                this.evenCeil(
    ...
    create(capacity: number, fpp: number) {
